...
var fieldIcon = new L.Icon({
  iconUrl: "https://img.icons8.com/material-two-tone/48/000000/house-with-a-garden.png",
  shadowUrl: 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/0.7.7/images/marker-shadow.png',
  iconSize: [32, 32],
  iconAnchor: [16, 16],
  popupAnchor: [1, -16],
...
var mymap = L.map('mapid').setView([44.4690744788 , 3.49143883359], 13);
...
for (var i = 0; i <latitudeOffre.length; i++){
  ...
  marcker = new L.marker([platitudeOffre, plongitudeOffre], { icon: sheepIcon }).bindPopup('Troupeau').addTo(mymap);
}
...
for (var i = 0; i <latitudeTerrain.length; i++){
  ...
  marcker = new L.marker([plattitudeTerrain, plongitudeTerrain], { icon: fieldIcon }).bindPopup('Terrain').addTo(mymap);
}